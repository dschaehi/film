...
from vr.models import ModuleNet, Seq2Seq, CnnModel, LstmModel, CnnLstmModel, CnnLstmSaModel, FiLMedNet, FiLMGen
...
def load_vocab(path):
    with open(path, "r") as f:
        ...
        if "program_token_num_inputs" not in vocab:
            vocab["program_token_num_inputs"] = {
                token: get_num_inputs(token) for token in vocab["program_token_to_idx"]
            }
        assert sorted(vocab["program_token_num_inputs"]) == sorted(
